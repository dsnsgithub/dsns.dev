  ...
  useEffect(() => {
    async function run() {
      try {
        ...
        const rawEvents = res.data.filter((e: any) => !ignoredTypes.includes(e.type));
        ...
        for (const event of rawEvents) {
          ...
          if (type === "PullRequestEvent" || type === "PullRequestReviewEvent") {
            ...
            tempEvents.push({
              ...
              verb: "forked",
              object: repo.name,
              description: `Original repository created by ${payload.forkee.full_name}`,
  ...
  return (
    <div className="flex flex-col space-y-3">
      {events.map((item) => (
        <div
          key={item.id}
          className="group mx-2 rounded-xl border border-transparent bg-viola-50 p-4 transition-all hover:border-viola-200 hover:bg-viola-100/40 sm:mx-4 sm:p-5"
        >
          <a href={item.url} target="_blank" rel="noopener noreferrer" className="flex items-start space-x-3 sm:space-x-4">
            <img
              ...
              className="h-8 w-8 shrink-0 rounded-full border-2 border-white shadow-sm sm:h-10 sm:w-10"
            />
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex flex-col sm:mb-0 sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="mt-1 shrink-0 text-[10px] text-gray-400 sm:mt-0 sm:text-xs">
                  {item.timestamp}
                </span>
              </div>
              ...
              {item.description && (
                ...
                >
                  <p className="line-clamp-3 whitespace-pre-wrap text-xs leading-relaxed text-gray-600">
                    {item.description}
                  </p>
                </div>
              )}

              <p className="mt-2 truncate text-[10px] font-semibold tracking-wide text-viola-700/80 sm:text-[11px]">
                {item.repo}
              </p>
            </div>
          </a>
